...
Given('{actor} is at the base url', (actor: Actor) =>
  actor.attemptsTo(ChangeApiConfig.setUrlTo('http://localhost:8080/webapp'))
)

When(
  '{pronoun} wants to create a new message with author {string} and message {string}',
...
Then(
  '{pronoun} is able to create the new message author {string} and message {string}',
  async (actor: Actor, author: string, message: string) => {
    ...
    await actor.attemptsTo(
      ...
      Ensure.that(
        Property.of(LastResponse.body<MessageDto>()).message.answeredBy(actor),
        equals(message)
      )
    )
  }
...
When('{pronoun} want to get a single message', (actor: Actor) =>
  actor.attemptsTo(ToPerform.getSingleMessage(), Log.the(LastResponse.body()))
)

Then('{pronoun} is able to get a single message', (actor: Actor) =>
  actor.attemptsTo(Ensure.that(LastResponse.status(), equals(200)))
)

When('{pronoun} view all messages', (actor: Actor) =>
  actor.attemptsTo(ToPerform.viewAllMessage(), Log.the(LastResponse.body()))
)
...
When(
  '{pronoun} update a message with author {string} and {string}',
  (actor: Actor, author: string, message: string) =>
    actor.attemptsTo(ToPerform.updateMessage(author, message))
)
